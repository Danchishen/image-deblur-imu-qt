...
QImage *ImageUtils::buildKernelImage(const MotionBlur* motionBlur) {
    ...
    double motionLength = motionBlur->radius * 2;
//    double motionAngle = motionBlur->angle;
    QVector< QVector <double> > xyVector= motionBlur->vectorXYb;
    ...
    if(xyVector.size() > 0){
        ...
        xMin = xyVector.at(0).at(0);
        xMax = xyVector.at(0).at(0);
        yMin = xyVector.at(0).at(1);
        yMax = xyVector.at(0).at(1);

        for (int i = 1; i < xyVector.size(); ++i) {
            if(xMin > xyVector.at(i).at(0)) xMin = xyVector.at(i).at(0);
            if(xMax < xyVector.at(i).at(0)) xMax = xyVector.at(i).at(0);
            if(yMin > xyVector.at(i).at(1)) yMin = xyVector.at(i).at(1);
            if(yMax < xyVector.at(i).at(1)) yMax = xyVector.at(i).at(1);
        }
        xLenght = fabs(xMax - xMin);
        yLenght = fabs(yMax - yMin);

        double scale;//коэффициент приведения размера троектории движения к размеру картинки
        if(xLenght > yLenght)
            scale = motionLength/2/xLenght;
        else
            scale = motionLength/2/yLenght;

        double center = 0.5 + size/2;
        QVector<QPointF> v_chart;
        for (int i = 0; i < xyVector.size(); ++i) {
            QPointF pointXY;
            pointXY=QPointF(center-xyVector.at(i).at(1)*(1)*scale,center-xyVector.at(i).at(0)*(1)*scale);
            v_chart.push_back(pointXY);
        }
        if(v_chart.size()>0){
            kernelPainter.drawPolyline(v_chart);
            //kernelPainter.drawLine(0, 0, center*1.5, center*1.5);
        }
    }
